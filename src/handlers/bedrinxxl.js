...
const extractor = async (page) => {
    ...
    const data = await page.evaluate(() => {
        ...
        try {
            ...
            const price = document.querySelector('.spanFiyat').innerText.replace('₺', '').trim()
            ...
            const size = Array.from(document.querySelectorAll('.eksecenekSelect option')).map(m=>m.innerText).filter(Number).map(m=> parseInt(m))
            const sku = document.querySelector('.productcode').innerText
            return {
